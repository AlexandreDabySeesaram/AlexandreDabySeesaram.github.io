...
::: {.mybox-body}
* Mechanics is assumed to be a key factor in the development of the disease
  * A vicious circle has been hypothesized [@liu_feedback_2010] where
    * Large stresses lead to
    * Fibrosis spread
    * In turn, this leads to stiffer zones and higher stresses
:::
...
* <g> Mechanical model of the lung </g> [@patte_quasi-static_2022]

:::
::: {.fragment .fade-in}


* <o> Uncertainty quantification </o> 

:::
::: {.fragment .fade-in}

* <r> Real-time estimation of the parameters </r>
:::
...
## Lung poro-mechanics{auto-animate=true}
...
$$
         \begin{cases}
            \div \cdot \sigm  + \vect{f} = 0 \qquad & \mathrm{in} \ \Omega \\
            \sigm \cdot \vect{n} = \vect{F}  & \text{on } \partial \Omega_{N} \\
            \vect{u} = \vect{u}_d \qquad \qquad & \text{on } \partial \Omega_{d} \\
            % \sigm = \ftensor{C}\left(\para\right):\eps(\vect{u}) & \mathrm{in} \ \Omega
         \end{cases}
         \label{eq:MechPb}
...
::: {data-id="1" }
  ...
  * $\matrice{\Sigma} = \frac{\partial \Psi}{\partial \matrice{E}}$
    * Non-linear finite strain poro-mechanics [@patte_quasi-static_2022]
:::
  * $\matrice{\Sigma}$ Second Piola-Kirchhoff stress tensor
  * $\matrice{E}$ is the Green-Lagrange strain tensor
...
## Simple mechanical problem - surrogate modelling{auto-animate=true}
...
::: {data-id="1" }
  * $\sigm = \ftensor{C}\left(\para\right):\eps(\vect{u})$
:::
...
* An approximation is thought after in the finite admissible (resp. admissible to zero) space

$$
    \mathcal{U}_h \left(\text{resp.} \mathcal{U}_h^0 \right) = \left\{\vect{u}_h  \; | \; \vect{u}_h \in \text{Span}\left( \left\{ N_i^{\Omega}\left(\vect{x} \right)\right\}_{i \in \llbracket 1,N\rrbracket} \right)^d \text{, }
...
::: {.column  width="50%"}
![Illustration of Finite Element Neural Network Interpolation (FENNI)](Figures/HiDeNN_Space_2.svg){width=500}
...
**Low-rank** approximation of the solution to avoid the **curse of dimensionality**
...
::: {.fragment .fade-up}
* Finding the tensor decomposition by **minimising** the **strain energy**
<!-- $$
...
$$
\vect{u}\left(\vect{x}, \para\right) = \textcolor{RougeLMS}{\sum\limits_{i=1}^{2}} \overline{\vect{u}}_i(\vect{x}) ~\prod_{j=1}^{\beta}\lambda_i^j(\mu^j) $$
:::
::: {.fragment fragment-index=3 .fade-in}
::: {data-id="2" }
$$
\vect{u}\left(\vect{x}, \para\right) = \sum\limits_{i=1}^{\textcolor{RougeLMS}{m}} \overline{\vect{u}}_i(\vect{x}) ~\prod_{j=1}^{\beta}\lambda_i^j(\mu^j) $$
:::
:::
:::
<style>
...
+--------------------------------------+
|  2. **Training and convergence proprieties**  |
+--------------------------------------+
...
![NeuROM](Figures/Movies/BiSliders_2D_snappy.mov){.absolute left="70" height="394" loop="true" autoplay=true}
...
::: {.column width="65%"}
![Multi-level training convergence](Figures/Multi_level_NeuROM.svg){width=500}
:::
...
* Extends the idea put forward by [@giacoma_toward_2015]
:::
...
:::: {style='margin-top: -3em;'}
::::
![Bi-stiffness NN-PGD parametrisation](Figures/2D_example_BiStiffness.svg){width=400}
...
::: {.mybox-body}
* Implementation of the poro-mechanics lung model [@patte_quasi-static_2022]
* Parametrisation of the segmented geometries
  * Variational encoder to reduce the segmentation's parametrisation
...
* Publication of the code ...
  * ... to come shortly
    * Do not hesitate to check the [**Github page**](https://github.com/AlexandreDabySeesaram?tab=repositories)
